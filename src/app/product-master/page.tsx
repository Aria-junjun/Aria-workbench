    ...
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-line bg-paper-warm/40 p-4 text-sm">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <SourceNote title="产品表现数据" detail="聚水潭月度表导入实发数量、实退数量、退货率和 ERP 成本基准；净销量仅作辅助参考，不做财务利润核算。" />
        ...
        <div className="overflow-x-auto rounded-xl border border-line bg-white">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-line">{inboundGroups.map((group) => {
              ...
              return <Fragment key={group.familyKey}><tr className="hover:bg-paper-warm/50"><td className="px-4 py-3"><div className="flex items-center gap-2"><button aria-label={`${expanded ? "收起" : "展开"}${group.productName} SKU`} className="rounded border border-line px-1.5 py-0.5 text-xs text-slate-500" onClick={() => setExpandedFamilies((current) => ({ ...current, [group.familyKey]: !expanded }))} type="button">{expanded ? "−" : "+"}</button>{product ? <Link className="font-medium text-action hover:underline" href={`/products/${product.id}`}>{group.productName}</Link> : <span className="font-medium text-slate-700">{group.productName}</span>}</div><div className="mt-1 pl-8 text-xs text-slate-400">{product?.category || "待补类目"}</div></td><td className="px-4 py-3">{productSkus.length}</td><td className="max-w-56 px-4 py-3 text-xs text-slate-600">{productSkus.slice(0, 4).map((sku) => sku.internalSkuCode).join("、") || "待关联"}{productSkus.length > 4 ? "…" : ""}</td><td className="px-4 py-3 text-xs">主供 {plan.primarySuppliers.length} · 备供 {plan.backupSuppliers.length}<MissingSupplyLink productId={product?.id} count={plan.missingFields.length} /></td><td className="px-4 py-3">{product?.portfolioStatus === "active" ? "继续经营" : product?.portfolioStatus === "optimize" ? "需要优化" : product?.portfolioStatus === "discontinued" ? "淘汰" : "观察"}</td><td className="px-4 py-3 text-xs text-slate-500">{metricSummary.source === "pending" ? "待采集" : `实发数量 ${metricSummary.monthlySales ?? 0}`}<div className="mt-1">{metricSummary.erpCostPrice != null ? `ERP成本基准 ${metricSummary.erpCostPrice}` : "ERP成本待采集"}</div><div className="mt-1">{metricSummary.returnRate != null ? `退货率 ${metricSummary.returnRate}%` : "退货率待采集"}</div><div className="mt-1 text-[11px] text-slate-400">来源：{metricSummary.source === "pending" ? "待录入" : "聚水潭月度表"}</div>{comparison.delta.monthlySales != null ? <div className={`mt-1 text-[11px] ${comparison.delta.monthlySales >= 0 ? "text-emerald-700" : "text-red-600"}`}>较上月实发数量 {comparison.delta.monthlySales >= 0 ? "+" : ""}{comparison.delta.monthlySales}</div> : null}</td></tr>{expanded ? <tr className="bg-paper-warm/30"><td className="px-4 py-3" colSpan={6}><div className="mb-2 text-xs text-slate-500">录入 {selectedPeriod}；主销售量按实发数量，聚水潭导入数据保存为历史快照，产品主表不做财务利润核算。</div><div className="space-y-2">{productSkus.map((sku) => { const draft = draftFor(sku); return <div className="rounded border border-line bg-white px-3 py-2" key={sku.id}><div className="flex flex-wrap items-center gap-2 text-xs"><span className="font-medium text-slate-800">{sku.internalSkuCode}</span><span className="text-slate-500">{sku.specification || "规格待补"}</span></div><div className="mt-2 grid gap-2 sm:grid-cols-3 lg:grid-cols-6"><SkuMetricInput label="实发数量" value={draft.monthlySales} onChange={(value) => updateDraft(sku.id, "monthlySales", value)} /><SkuMetricInput label="净销售额（参考）" value={draft.salesAmount} onChange={(value) => updateDraft(sku.id, "salesAmount", value)} /><SkuMetricInput label="ERP成本价" value={draft.erpCostPrice} onChange={(value) => updateDraft(sku.id, "erpCostPrice", value)} /><SkuMetricInput label="实退数量" value={draft.returnQuantity} onChange={(value) => updateDraft(sku.id, "returnQuantity", value)} /><SkuMetricInput label="退货率%" value={draft.returnRate} onChange={(value) => updateDraft(sku.id, "returnRate", value)} /></div></div>; })}</div></td></tr> : null}</Fragment>;
            })}</tbody>
          </table>
        </div>
      )}
...
function MissingSupplyLink({ productId, count }: { productId?: string; count: number }) {
  if (!count) return null;
  const label = `待处理 ${count} 项`;
  return productId ? <Link className="mt-1 block text-amber-700 underline decoration-dotted underline-offset-2 hover:text-amber-900" href={`/products/${productId}`} title="进入产品详情，查看具体 SKU 的供应方案缺项">{label}</Link> : <div className="mt-1 text-amber-700">{label}</div>;
}
